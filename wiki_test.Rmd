...
```{r}
library(scales)
ggplot(foo, aes(x = dates, y = n, color = pick)) +
  geom_vline( xintercept = keydates$keydate_dates) +
  geom_point( alpha = 0.2) +
  geom_line(aes(y=rollmean(n, 14, na.pad=TRUE)), size =1.2) +

  labs(y = "edit count") +
  lims(x = as.Date(c("2020-01-01", "2020-09-01")), y = c(0,60))+
...
april 30 - vetting committee anounced
aug 11 - selection of Harris

```{r}
keydate_dates <- as.Date(c("2020-03-15", "2020-04-08", "2020-04-30", "2020-08-11"))
keydate_events <- c("biden promises a woman", "Biden become presumptive nominee", "vetting committee anounced", "selection of Harris")
keydates <- data.frame(keydate_dates, keydate_events)
```
